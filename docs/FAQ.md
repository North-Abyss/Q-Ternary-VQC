...
This model is designed as a *decision support system*, not an autonomous diagnostic tool. In clinical practice, high recall (sensitivity) is prioritized to avoid missing true cancer cases. Our model achieves a **90.41% F1 score**, indicating strong clinical recall. Furthermore, the SHAP explainability module provides doctors with the exact reasons behind every prediction, ensuring human oversight over the AI's suggestions.
