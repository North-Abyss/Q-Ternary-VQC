...
import '../widgets/patient_intake_form.dart';
...
class _DashboardPageState extends State<DashboardPage> {
              ...
              builder: (context, constraints) {
                if (constraints.maxWidth > 900) {
                  return Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Expanded(flex: 2, child: _buildRunHistory()),
                      const SizedBox(width: 24),
                      Expanded(flex: 1, child: _buildRightColumn(themeProvider)),
                    ],
                  );
                } else {
                  return Column(
                    children: [
                      _buildRunHistory(),
                      const SizedBox(height: 24),
                      _buildRightColumn(themeProvider),
                    ],
                  );
                }
              },
              ...
              ? Padding(
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.history_toggle_off, size: 80, color: Theme.of(context).colorScheme.primary.withOpacity(0.2)),
                        const SizedBox(height: 24),
                        const Text('No pipeline runs recorded yet.', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 8),
                        const Text('Head over to the Pipeline tab to configure and launch your first quantum model training!', style: TextStyle(color: Colors.grey), textAlign: TextAlign.center),
                      ],
                    ),
                  ),
                )
  ...
  Widget _buildRightColumn(ThemeProvider themeProvider) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
